...
public protocol CloudKitPersonDatabaseDelegate {
    func cloudKitNoteRecordChanged(record: CKRecord)
}

public class CloudKitPersonDatabase {
    
    static let shared = CloudKitPersonDatabase()
    private init() {
        let zone = CKRecordZone(zoneName: "person-zone")
        zoneID = zone.zoneID
    }
    
    public var delegate: CloudKitPersonDatabaseDelegate?
    ...
    public func saveSubscription() {
        ...
        let subscription = CKQuerySubscription(recordType: "Person",
                                               predicate: predicate,
